    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff >= self.threshold:
            ...
            while not end_episode:
                ...
                if not np.isnan(Q[s_current]).all():
                    action = np.nanargmax(Q[s_current])
                else:
                    
                    action = random.choice(list_pos)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                q_update(Q, Q_old, self.alpha, self.gamma, s_current, action, R, s_next)
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
...
def q_update(q, q_old, alpha, gamma, st, at, rt, st1):
    q[st][at] = q[st][at] + alpha * (rt + gamma * max(q_old[st1]) - q[st][at])
    return q
